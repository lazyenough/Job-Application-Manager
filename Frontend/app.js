...
const fetchOptions = (method = "GET", body = null) => {
    const options = {
        method: method, 
        headers: {},
        // CRITICAL: Tells the browser to send cookies to the backend 
        // AND allow the backend to save new cookies on the frontend
        credentials: "include" 
    };
    if (body) {
...
ingestBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/jobs/ingest/preview`, fetchOptions("POST", { job_url: jobUrl }));
...
saveModalBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/jobs/ingest`, fetchOptions("POST", editedJobData));
...
async function updateJobStatus(jobId) {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/jobs/${jobId}`, fetchOptions("PATCH", { status: newStatus }));
...
async function deleteJob(jobId) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/jobs/${jobId}`, fetchOptions("DELETE"));
        if (response.ok) {
...
logoutBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        if (response.ok) {
            // Drop credentials and boot them backward to the portal login screen
            window.location.href = "loginPage/login.html";
        } else {
            errorBox.textContent = "Logout failed on server.";
            errorBox.style.display = "block";
        }
    } catch (error) {
        console.error("Logout execution failure:", error);
        errorBox.textContent = "Network error while signing out.";
        errorBox.style.display = "block";
    }
});
